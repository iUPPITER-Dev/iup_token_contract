...
mod tests {
    ...
    use cosmwasm_std::{from_json, Addr, Coin, DepsMut, MessageInfo, Uint128};
    ...
    fn query_all_spender_allowances_works() {
        ...
        let info = create_message_info(owner1.as_ref(), vec![]);
        let env = mock_env();
        ...
        let info = create_message_info(owner2.as_ref(), vec![]);
        let env = mock_env();
    ...
    fn query_all_accounts_works() {
        ...
        let info = create_message_info(acct1.as_ref(), vec![]);
        let env = mock_env();
        execute(
            deps.as_mut(),
            env.clone(),
