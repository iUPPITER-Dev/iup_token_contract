...
use crate::fee::{add_fee_attributes, calculate_fee};

#[cfg(test)]
use crate::fee::apply_fee_transfers;

use crate::error::ContractError;
use crate::state::{ALLOWANCES, ALLOWANCES_SPENDER, BALANCES, FEE_CONFIG, TOKEN_INFO};
...
pub fn execute_transfer_from(
    ...
    // 수수료 계산 (새로운 방식)
    let fee_result = calculate_fee(deps.as_ref(), amount, &owner_addr)?;
...
pub fn execute_send_from(
    ...
    // Cw20ReceiveMsg 메시지 생성 및 추가
    let msg = Cw20ReceiveMsg {
        sender: info.sender.into(),
        amount: fee_result.transfer_amount,  // 수수료 차감 후 금액
        msg,
    }
...
mod tests {
    ...
    use cosmwasm_std::{ Coin, CosmosMsg, SubMsg, Timestamp, WasmMsg};
    ...
    fn get_balance<T: Into<String>>(deps: Deps, address: T) -> Uint128 {
     ...
     // MessageInfo를 생성하는 헬퍼 함수
     fn create_message_info(sender: &str, funds: Vec<Coin>) -> MessageInfo {
        MessageInfo {
    ...
    fn do_instantiate<T: Into<String>>(
        ...
        let info = create_message_info("creator", vec![]);
        let env = mock_env();
    ...
    fn increase_decrease_allowances() {
        ...
        let info = create_message_info(owner.as_ref(), vec![]);
        let env = mock_env();
    ...
    fn allowances_independent() {
        ...
        let info = create_message_info(spender.as_ref(), vec![]);
        let env = mock_env();
